...
type DeviceCtlService struct {
	logger *slog.Logger
	dbCon  *database.PostgresService
	cctvId string
}

func NewDeviceCtlService(dbCon *database.PostgresService) DeviceCtlService {
...
func (d *DeviceCtlService) GetServiceCapaCtl(res http.ResponseWriter, req *http.Request) {
	...
	endpoint, err := d.GetDeviceEndpointFromDatabase(cctvId)
	if err != nil {
	...
	device := ptz.DeviceConnect(endpoint.Endpoint) // TODO DB 조회
...
func (d *DeviceCtlService) GetDeviceStatusCtl(res http.ResponseWriter, req *http.Request) {
	...
	endpoint, getErr := d.GetDeviceEndpointFromDatabase(cctvId)
	if getErr != nil {
		response.Response(res, http.StatusInternalServerError, response.CommonResponseWithMessage{
			Status:  http.StatusInternalServerError,
			Code:    "STA002",
...
func (d *DeviceCtlService) GetDeviceConfigCtl(res http.ResponseWriter, req *http.Request) {
	...
	device := ptz.DeviceConnect(endpoint.Endpoint)
